...
Rust compiles to a static library linked into the C binary, and the boundary is generated in
both directions by two different tools:

- **Rust → C: `cbindgen`** (`rust/cbindgen.toml`, and the htp/ffi/jsonbuilder configs) produces
  `rust/gen/rust-bindings.h` & friends from the Rust FFI (`rust/src/**/ffi`). `make` runs it
  automatically; a Rust function C must call just needs to be exposed via the FFI layer.
- **C → Rust: `bindgen`** produces `rust/sys/src/sys.rs`, the `suricata-sys` crate, from
  `src/bindgen.h` (the list of C headers exported to Rust). Rust reaches engine API through
  `use suricata_sys::sys::{…}`. Unlike cbindgen output, `sys.rs` is **checked in and not
  regenerated by `make`**: after adding C API Rust needs, add the header to `src/bindgen.h` if
  missing, run `make -C rust update-bindings`, and commit the result (`make -C rust check` only
  *warns* when it's stale). Only allowlisted symbols cross — mainly `SC*` (see the
  `--allowlist-*` flags in `rust/Makefile.am`), one more reason new API must be `SC`-prefixed.
...
- **Banned libc functions** (use the safe replacement): `sprintf`→`snprintf`,
  `strcpy`→`strlcpy`, `strcat`/`strncat`→`strlcat`, `strtok`→`strtok_r`, `bzero`→`memset`;
  also avoid `rand`/`rand_r`, `strncpy`, `index`/`rindex`, `strndup`, `strchrnul`. The
  authoritative table is the "Banned functions" section of
  `doc/userguide/devguide/codebase/code-style.rst` — check it, and extend it when banning more.
- **String→number conversion**: never `atoi`/`atol`/`atoll` (no error reporting, no overflow
  detection, undefined on overflow) and don't hand-roll `strtol` checks. Use the checked
  helpers in `src/util-byte.h`: `StringParseUint8/16/32/64` and `StringParseInt*` (plus the
  `StringParse{U,I}<N>RangeCheck` variants for a bounded range, and `ByteExtractString*` when
  parsing a fixed-length, non-NUL-terminated slice of wire/rule data). They return a negative
  value on bad input and are sized to the destination type, so a malformed rule or config
  value fails loudly instead of silently becoming 0 or a truncated integer. In Rust, use the
  normal `str::parse` / `nom` numeric combinators.
- **Doxygen**: function documentation lives at the **definition in the `.c` file**
  (`/** \brief … \param … \retval … */`), **never above the declaration in the `.h`** — a
  header block describing what a function does, its parameters or its caveats belongs in the
  `.c`, even for public `SC*` API. Headers carry only lighter comments, for typedefs, macros,
  struct fields, and grouping/one-line notes on declarations. Two more rules follow from this:
  - When a function has **two definitions** — a real one and a compiled-out no-op stub behind
    an `#ifdef` (the pattern used all over `util-landlock.c`, `win32-*.c`, …) — the Doxygen
    block goes on the **real implementation**, and the stub stays bare.
  - Don't say the same thing twice in one file: if the doc block above the definition already
    explains *why* a flag/access/branch is there, drop the duplicate rationale comment from
    the function body. Body comments are for implementation detail the caller doesn't need.
