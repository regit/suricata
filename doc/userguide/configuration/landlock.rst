...
``load``
  nothing to declare. Datasets are loaded during startup, before the sandbox
  is enforced, so a ``load``-only rule keeps working whatever the Landlock
  configuration is.

``save``
  the set is written out while Suricata runs, replacing the whole file. Add
  its directory to ``security.landlock.directories.rewrite``: plain ``write``
  is enough to create the file the first time, but not to replace the content
  of an existing one, so the set would silently stop being updated from the
  second run on.
