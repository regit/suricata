 ...
 *
 * Intended for callers that resolve their file paths at configuration-parse
 * time, before the landlock ruleset exists. The pending entry is consumed by
 * LandlockSandboxing(), which calls SCLandlockGrantFile() for each
 * registration. Safe to call regardless of whether landlock is enabled at
 * runtime: unused registrations are freed at the end of LandlockSandboxing().
 *
 ...
 * subdirectories rooted at this path, and LANDLOCK_ACCESS_FS_REMOVE_FILE so
 * callers can unlink their own staged files. REMOVE is kept out of the
 * default write grant since it is a common anti-forensics primitive, so only
 * opt-in callers that manage their own private tree get it. Intended for
 * modules such as file-store that need to move files from a staging directory
 * into the final tree. Should be used only on directories fully owned by the
 * caller.
 *
 ...
 *
 * Same as SCLandlockGrantWritePath() but also grants
 * LANDLOCK_ACCESS_FS_REMOVE_FILE on the directory. Intended for modules that
 * legitimately recycle their own files (e.g. pcap-log ring buffer). REMOVE is
 * deliberately kept out of the default write grant because it is a common
 * anti-forensics primitive -- opt in explicitly.
 *
 * \param vruleset opaque landlock ruleset
 * \param directory directory to grant the access on
 */
 ...
 * directory. Intended for the unix command socket, which unlinks any stale
 * socket before bind(), hence the removal access. MAKE_SOCK is kept out of
 * the default write grant since a module that only connects to an existing
 * socket does not need it: eve-log's unix_stream/unix_dgram filetypes
 * connect() to a socket somebody else made, they never bind.
 *
 * \param vruleset opaque landlock ruleset
 * \param directory directory to grant the access on
 */
...
/**
 * \brief Grant a minimal per-file landlock rule (read/write/truncate)
 *
 * The target file is created (0644, O_NOFOLLOW) if missing when a write flag
 * is requested, so landlock can attach the rule to a real inode. Intended for
 * use from plugins, output modules and core subsystems that need per-file
 * access -- notably truncate-on-open (mode "w") on a single file without
 * opening truncate up on the whole parent directory. No-op when landlock is
 * not compiled in or the running kernel does not support it.
 *
 * \param vruleset opaque landlock ruleset
