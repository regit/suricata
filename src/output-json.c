        ...
        const Packet *p, const Flow *f, JsonBuilder *js, MemBuffer *payload)
{
    ...
    if (payload) {
        if (cfg->include_streamdata & (JSONLOG_JSON_PAYLOAD_BASE64|JSONLOG_JSON_PAYLOAD)) {
...
OutputInitResult OutputJsonInitCtx(ConfNode *conf)
{
    ...
    if (conf) {
        ...
        const ConfNode *fulldatap = ConfNodeLookupChild(conf, "full-logging-for-alerted-flows");
